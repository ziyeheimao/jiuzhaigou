  ...
  <section>
    <el-form :model="form" :rules="rules" ref="form" :label-width="labelWidth">
      <el-form-item label="窗口办理" prop="windowProcess">
        <el-radio-group v-model="form.windowProcess" :disabled="disabled">
      ...
      <div class="box">
        <el-form-item label="是否支持网上预约" prop="supNetAppoint">
          <el-radio-group v-model="form.supNetAppoint" :disabled="disabled">
            <el-radio v-for="(v,k) in res.supNetAppoint" :key="k" :label="v.label">{{v.name}}</el-radio>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="办事者到场次数" prop="presentTimes">
          <el-input-number v-model="form.presentTimes" :disabled="disabled"></el-input-number> &nbsp;次
        </el-form-item>
      </div>
      ...
      <div class="box">
        <el-form-item label="是否支持全省范围内就近申请" prop="supProvinceApply">
          <el-radio-group v-model="form.supProvinceApply" :disabled="disabled">
        ...
        <el-form-item label="是否支持全省范围内就近领取" prop="supProvinceReceive">
          <el-radio-group v-model="form.supProvinceReceive" :disabled="disabled">
      ...
      <div class="box">
        <el-form-item label="网上办理" prop="netProcess">
          <el-radio-group v-model="form.netProcess" :disabled="disabled">
            <el-radio v-for="(v,k) in res.netProcess" :key="k" :label="v.label">{{v.name}}</el-radio>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="网办类型" prop="netProcessType">
          <el-radio-group v-model="form.netProcessType" :disabled="disabled">
      ...
      <el-form-item label="认证等级要求" prop="authLevel">
        <el-radio-group v-model="form.authLevel" :disabled="disabled">
          <el-radio v-for="(v,k) in res.authLevel" :key="k" :label="v.label">{{v.name}}</el-radio>
        </el-radio-group>
      </el-form-item>

      <el-form-item label="是否下放" prop="isDown">
        <el-radio-group v-model="form.isDown" :disabled="disabled">
      ...
      <el-form-item label="下放单位" prop="downTown" v-if="form.isDown === 1">
        ...
        <el-checkbox-group v-model="form.downTown" @change="handleCheckedChange" :disabled="disabled">
          <el-checkbox v-for='(v,k) in res.downTown' :key="k" :label="v">{{v}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>

      <el-form-item label="办理查询" prop="proessSelect">
        <el-radio-group v-model="form.proessSelect" :disabled="disabled">
          <el-radio v-for="(v,k) in res.proessSelect" :key="k" :label="v.label">{{v.name}}</el-radio>
        </el-radio-group>
      </el-form-item>

      <el-form-item label="特别程序" prop="specialProgram">
        <el-input v-model="form.specialProgram" type="textarea" :rows='4' placeholder="请输入特别程序,无特别程序填写“无”" :disabled="disabled"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    init () {
      ...
      if (this.seeAuditId) {
        this.$api.workImplementationByWorkId(this.workId, {auditId: this.seeAuditId}).then(res => {
