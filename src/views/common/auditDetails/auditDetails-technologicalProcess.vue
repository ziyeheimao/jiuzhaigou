...
<template>
  <section class="ctn">
    <div class="btnTop">
      <el-button type="primary" icon="el-icon-plus" @click="addSearch" :disabled="disabled">关联流程图</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    addSearch () {
      this.quoteDialogVisible = true
      this.$api.activitiPage(this.relationForm).then(res => {
        if (res.code === 0) {
    ...
    relationSee (row) {
      ...
      this.$api.workActivitiById(row.id).then(res => {
        console.log(res)
      })
    },
    // 关联
