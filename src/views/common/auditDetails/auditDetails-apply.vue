<template>
  <section class="ctn">
    <div class="btnTop">
      <el-button-group>
        <el-button type="primary" icon="el-icon-plus" @click="applicationMaterialsTableAdd" :disabled="disabled">新增</el-button>
        <el-button type="primary" icon="el-icon-delete" @click="delsDialogVisible = true" :disabled="disabled">删除</el-button>
      </el-button-group>
    </div>
    ...
    <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="150">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="del(scope.row)" :disabled="disabled">删除</el-button>
          <el-button type="text" size="small" @click="tableEdit(scope.row)" :disabled="disabled">编辑</el-button>
          <el-button type="text" size="small" @click="relationSearch(scope.row)" :disabled="disabled">关联表单</el-button>
        </template>
      </el-table-column>
    </el-table>
      ...
      <el-table :data="relationTable" style="width: 100%">
        ...
        <el-table-column label="操作" width="150">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="relationCancel(scope.row)">取联</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="确认操作" :visible.sync="relationSeeDialogVisible" width="1000px" :append-to-body='true'>
      <div>
        <el-form :model="wFofm" :rules="rules" ref="wFofm" :label-width="'60px'" :inline='true'>

          <el-form-item label="姓名">
            <el-input v-model="wFofm.a"></el-input>
          </el-form-item>

          <el-form-item label="年龄">
            <el-input v-model="wFofm.q"></el-input>
          </el-form-item>

          <el-form-item label="籍贯">
            <el-input v-model="wFofm.w"></el-input>
          </el-form-item>

          <el-form-item label="户口">
            <el-radio-group v-model="wFofm.v">
              <el-radio label="农业户口"></el-radio>
              <el-radio label="城镇户口"></el-radio>
            </el-radio-group>
          </el-form-item>

          <el-form-item label="学历">
            <el-radio-group v-model="wFofm.r">
...
export default {
  ...
  data () {
    return {
      ...
      relationTable: [
        {id: 1, name: '静态', mark: 'aada', description: '描述'},
        {id: 2, name: '静态', mark: 'xxxx', description: '描述'},
        {id: 3, name: '静态', mark: 'xxxx', description: '描述'},
        {id: 4, name: '静态', mark: 'xxxx', description: '描述'},
        {id: 5, name: '静态', mark: 'xxxx', description: '描述'}
      ],
      relationSearchForm: {
        page: 1,
        limit: 10,
  ...
  methods: {
    ...
    relationSearch (row) {
      ...
      console.log(this.relationSearchForm)
      // 获取关联数据 / 显示模态框 / 渲染表格
      let req = {}
      for (let i in this.relationSearchForm) {
        if (this.relationSearchForm[i]) {
          req[i] = this.relationSearchForm[i]
        }
      }
      this.$api.formList(req).then(res => {
        if (res.code === 0) {
