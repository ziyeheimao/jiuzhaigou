...
export default {
  ...
  methods: {
      ...
      switch (type) {
        ...
        case 4:
          ...
          this.$api.workUpdateStatus(req).then(res => {
            if (res.code === 0) {
              openInfo('上报成功 (๑•̀ㅂ•́)و✧', 's')
              this.$router.push({name: 'listMattersAdmin'})
              sessionStorage.setItem('stepsActive', 0)
    ...
    add (form) {
      this.$refs[form].validate((valid) => {
        if (valid) {
          this.$api.workQuestionAdd(this.form).then(res => {
            if (res.code === 0) {
              this.form.question = '' // 问题
              this.form.answer = '' // 解答
              openInfo('保存成功', 's')
            } else {
    ...
    dels () {
      ...
      .then(() => {
        ...
        this.$api.workQuestionDelByIds(obj).then(res => {
          if (res.code === 0) {
            openInfo('删除成功', 's')
            this.search() // 刷新
          } else {
    ...
    // 删除
    del (row) {
      this.$api.workQuestionDelById(row.id).then(res => {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          let req = {
            ...
            question: this.editForm.question, // 问题
            answer: this.editForm.answer // 解答
          }
          this.$api.workQuestionUpdata(req).then(res => {
            if (res.code === 0) {
  ...
  beforeCreate () {},
  created () {
    this.form.workId = Number(sessionStorage.getItem('workId'))
    this.search()
  },
  beforeMount () {},
  mounted () {},
