  ...
  <section class="ctn">
    <header>
      ...
      <el-form :model="form" :rules="rules" ref="form" :label-width="labelWidth" :inline='true'>
        <el-form-item label="申请人/单位:" prop="">
          <el-input style="width: 260px;" v-model="form.name" placeholder="请输入申请人/单位"></el-input>
        </el-form-item>
        ...
        <el-form-item label="办理渠道:" prop="">
          <el-select style="width: 260px;" v-model="form.handlingChannels">
            <el-option v-for="(v,k) in res.handlingChannels" :key="k" :label="v.label" :value="v.value"></el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="超时情况:" prop="">
          <el-checkbox-group v-model="form.overtime">
            <el-checkbox v-for="(v,k) in res.overtime" :key="k" :label="v"></el-checkbox>
          </el-checkbox-group>
        </el-form-item>
    ...
    <main>
      ...
      <el-tabs class="tabs" v-model="activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="受理中-已驳回" name="2"></el-tab-pane>
        <el-tab-pane label="办理成功" name="3"></el-tab-pane>
        <el-tab-pane label="办理失败" name="4"></el-tab-pane>
      </el-tabs>

      <div class="link">
        <el-link @click='clickLink(1)' icon="el-icon-circle-plus-outline">新增</el-link>
        <el-link @click='clickLink(2)' icon="el-icon-delete">删除</el-link>
      </div>
      ...
      <div class="bom">
        <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column align='center' prop="b" label="法定总时限超时(工作日)"></el-table-column>
          <el-table-column align='center' prop="c" label="岗位超时(工作日)"></el-table-column><!-- :formatter='tableFilter' -->
          <el-table-column align='center' prop="d" label="申请人/单位"></el-table-column>
          <el-table-column align='center' prop="e" label="当前节点"></el-table-column>
          <el-table-column align='center' prop="f" label="办理状态"></el-table-column>
          <el-table-column align='center' prop="g" label="事项名称"></el-table-column>
          <el-table-column align='center' prop="h" label="事项类型"></el-table-column>
          <el-table-column align='center' prop="i" label="证件类型"></el-table-column>
          <el-table-column align='center' prop="j" label="证件号"></el-table-column>
          <el-table-column align='center' prop="k" label="办理渠道"></el-table-column>
          <el-table-column align='center' prop="l" label="申请时间"></el-table-column>
          ...
          <el-table-column align='center' :formatter='tableFilter' label="操作" width="150">
            <template slot-scope="scope">
              ...
              <el-button type="text" size="small" @click="tableClick(1, scope.row)">办理</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  data () {
    return {
      ...
      tableData: [
        {a: '512001041003', b: '1天', c: '未超时', d: '王凯', e: '草稿', f: '草稿', g: '教师资格证相关政策书面咨询', h: '行政许可', i: '居民身份证', j: '412728199893422', k: '线上申请', l: '2020-2-10'},
        {a: '512001041003', b: '1天', c: '未超时', d: '王凯', e: '草稿', f: '草稿', g: '教师资格证相关政策书面咨询', h: '行政许可', i: '居民身份证', j: '412728199893422', k: '线上申请', l: '2020-2-10'},
        {a: '512001041003', b: '1天', c: '未超时', d: '王凯', e: '草稿', f: '草稿', g: '教师资格证相关政策书面咨询', h: '行政许可', i: '居民身份证', j: '412728199893422', k: '线上申请', l: '2020-2-10'},
        {a: '512001041003', b: '1天', c: '未超时', d: '王凯', e: '草稿', f: '草稿', g: '教师资格证相关政策书面咨询', h: '行政许可', i: '居民身份证', j: '412728199893422', k: '线上申请', l: '2020-2-10'}
      ]
  ...
  methods: {
    init () {
      ...
      this.$api.sysDictSelectItemsByDictType({dictType: 'handlingChannels'}).then(res => { // 证件类型
        if (res.code === 0) {
          for (let i of res.bean) {
            this.res.handlingChannels.push({value: i.sortNo, label: i.dictItemName})
          }
        }
      })

      this.$api.sysDictSelectItemsByDictType({dictType: 'overtime'}).then(res => { // 超时
        if (res.code === 0) {
          for (let i of res.bean) {
            this.res.overtime.push(i.dictItemName)
            this.form.overtime.push(i.dictItemName)
          }
        }
      })
    },
